 ...
 *
 * FOR DOCUMENTATION AND MORE EXAMPLES: VISIT http://Debug.ospinto.com
 *
 *
 * PURPOSE
 * =============
...
class Debug {
	...
	function __construct($var,$forceType="",$bCollapsed=false) {
		...
		if(!defined('BDebugINIT')) {
			define("BDebugINIT", TRUE);
			$this->initJSandCSS();
		}
	...
	function varIsDBResource($var,$db="mysql") {
		...
		echo "<tr><td class=\"Debug_resourceKey\">&nbsp;</td>";
		for($i=0;$i<$numfields;$i++) {
			...
			echo "<td class=\"Debug_resourceKey\" title=\"".$field_header."\">".$field_name."</td>";
		}
		echo "</tr>";
		for($i=0;$i<$numrows;$i++) {
			$row=call_user_func($db."_fetch_array",$var,constant(strtoupper($db)."_ASSOC"));
			echo "<tr>\n";
			echo "<td class=\"Debug_resourceKey\">".($i+1)."</td>"; 
